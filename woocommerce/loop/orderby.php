...
<form class="form-inline form-floating woocommerce-ordering" method="get">
<?php if ( $use_label ) : ?>
		<label for="woocommerce-orderby-<?php echo esc_attr( $id_suffix ); ?>"><?php echo esc_html__( 'Sort by', 'woocommerce' ); ?></label>
	<?php endif; ?>
	<select 
		name="orderby" 
		class="custom-select form-select orderby rounded-3" aria-label="<?php esc_attr_e( 'Shop order', 'woocommerce' ); ?>"
		<?php if ( $use_label ) : ?>
...
---- ORDERBY EVERTON ---- NO BORRAR ----
<form class="form-inline woocommerce-ordering" method="get">
    <select name="orderby" class="custom-select my-1 mr-sm-2 orderby" aria-label="<?php esc_attr_e( 'Shop order', 'woocommerce' ); ?>">
		<?php foreach ( $catalog_orderby_options as $id => $name ) : ?>
			<option value="<?php echo esc_attr( $id ); ?>" <?php selected( $orderby, $id ); ?>><?php echo esc_html( $name ); ?></option>
		<?php endforeach; ?>
	</select>
